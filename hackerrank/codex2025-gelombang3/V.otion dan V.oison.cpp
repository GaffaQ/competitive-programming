...
void solve() {
    ...
    auto calc = [&](ll V) -> ll {
        ...
        for (ll j = 0; j <= limit; j++) {
            ll term1 = C(m, j);
            ll term2 = C(V - j * (y - x + 1) + m, m); 
            ll term = mod_mul(term1, term2, MOD);
            
            if (j % 2 == 1) {
                ans = mod_sub(ans, term, MOD);
            } else {
                ans = (ans + term) % MOD;
            }
        }
    ...
    ll ways = mod_sub(calc(maks), calc(mins - 1), MOD);
    
    ll ans = mod_mul(ways, C(n, m), MOD);
